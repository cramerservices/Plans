...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const serviceStreet = pickMeta(md, "service_street", "street_address");
    ...
    if (portalCustomer?.id && crmCustomer.portal_customer_id !== portalCustomer.id) {
      ...
      if (error) {
        return json(corsHeaders, 500, {
          error: "CRM customer portal link update failed",
          details: error.message,
        });
      }

      crmCustomer.portal_customer_id = portalCustomer.id;
    }
    ...
    if (!membership) {
      ...
      if (error) {
        return json(corsHeaders, 500, {
          error: "membership lookup by user_id failed",
          details: error.message,
        });
      }
